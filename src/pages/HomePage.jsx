  ...
  return (
    <div style={{ background: '#faf9f6', color: '#1a1c1a' }} className="min-h-screen font-manrope">
            ...
            <div className="pt-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4 max-w-xl">
                ...
                >
                  ...
                  <p className="font-noto text-2xl md:text-3xl text-[#1a1c1a] group-hover:text-white mb-2 transition-colors leading-tight" style={{ fontWeight: 700 }}>
            ...
            <div className="bg-[#faf9f6]/70 backdrop-blur-sm border border-[#d1c5b4]/30 p-10">
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <p className="font-manrope text-xs tracking-[0.15em] uppercase text-[#7f7667] mt-2">
                    {isRu ? 'Лет на рынке' : 'Years in business'}
                  </p>
                </div>
              </div>
            </div>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 right-6 z-40 w-14 h-14 satin-gradient rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-110"
        aria-label="WhatsApp"
      >
        ...
        <div className="space-y-0">
          {t.faq.items.map((faq, i) => (
            <div key={i} className="border-b border-[#d1c5b4]/40">
              <button onClick={() => toggle(i)} className="w-full flex items-center justify-between py-6 text-left transition-colors group">
                ...
                <span className={`font-noto text-2xl text-[#775a19] shrink-0 transition-transform duration-300 ${openIndex === i ? 'rotate-45' : ''}`}>+</span>
              </button>
              <div className={`faq-content ${openIndex === i ? 'open pb-6' : ''}`}>
                <p className="font-manrope text-[#4e4639] leading-relaxed max-w-2xl">{faq.a}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white p-8 md:p-14">
          <div className="flex gap-1 mb-10">
            {[...Array(totalSteps)].map((_, i) => (
              <div key={i} className={`h-0.5 flex-1 transition-all duration-500 ${i <= currentStep ? 'bg-[#775a19]' : 'bg-[#d1c5b4]/40'}`} />
            ))}
          </div>
