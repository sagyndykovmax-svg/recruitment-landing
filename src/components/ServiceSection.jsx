    ...
    <section className="py-20 lg:py-28 bg-gray-50 border-t-4 border-mint-400" id="service">
      <div className="max-w-6xl mx-auto px-4" ref={ref}>
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-mint-600 font-semibold text-sm uppercase tracking-wider">{s.tag}</span>
        </div>
        <h2 className="text-3xl sm:text-4xl font-bold mb-4">{s.title}</h2>
        <p className="text-gray-500 text-lg mb-12 max-w-2xl">{s.subtitle}</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-16">
          {[
            { value: s.salary, label: s.salaryDesc, color: 'text-mint-600' },
            { value: s.housing, label: s.housingDesc, color: 'text-gray-900' },
            { value: s.noExp, label: s.noExpDesc, color: 'text-gray-900' },
            { value: s.contract, label: s.contractDesc, color: 'text-gray-900' },
          ].map((item, i) => (
        ...
        <div className="bg-gradient-to-r from-mint-400/10 to-white border border-mint-400/20 rounded-2xl p-8 mb-12">
          <h3 className="text-xl font-bold mb-6">{s.reqTitle}</h3>
